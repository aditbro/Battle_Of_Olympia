...
{
    if (VillageList_empty(L)){
        return Alokasi_point(village_pos, indeks);
    }
    ...
    /* Display village index and village position */
{
    if (!VillageList_empty(L)){
        ...
        show_village_in_list(Build(M, Absis(village_pos), Ordinat(village_pos)));
        printf("\n");
        Display_village_list(M, VillageTail(L));
    }
}

void show_village_in_list(Build U){
/* Function to show village data */
    printf("  ");
    TulisPOINT(Build_Pos(U));
}
